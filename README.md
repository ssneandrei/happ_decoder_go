...
Приложение полностью локально расшифровывает конфигурации (VLESS, VMess, Shadowsocks, Trojan) и отдаёт их в виде чистого списка для использования в **Forkop**, **Podkop**,**OpenClash**.
...
* **Интерактивный LuCI UI:**
  ...
  * Генерация прямых ссылок подписки для Forkop/Podkop.
...
```bash
sh -c "$(wget -qO- https://raw.githubusercontent.com/ssneandrei/happ_decoder_go/main/install.sh)"
```
...
```bash
sh -c "$(wget -qO- https://raw.githubusercontent.com/ssneandrei/happ_decoder_go/main/uninstall.sh)"
```
